...
public:
	...
	*/
	...
	/**
	*	@brief	Tree의 root 포인터를 공유한다.
	*	@pre	none
	*	@post	Tree의 root 포인터를 pointer매개변수에게 공유해준다.
	*/
...
{
	GetPointing(root, item, found, point);		// Tree 검색 함수 호출
}
...
// node를 검색 후 해당 node를 가리키는 포인터를 공유한다
template<class T>
void GetPointing(Node<T> *root, T& item, bool &found, Node<T>*& point)
{
	if (root == NULL)						// root가 NULL인 경우 found는 false 
		...
		GetPointing(root->left, item, found, point);	// 왼쪽 노드로 가서 retrieve 함수 호출
	else if (item > root->data)				// 찾고자 하는 아이템값이 root값보다 클 때
		GetPointing(root->right, item, found, point);	// 오른쪽 노드로 가서 retrieve 함수 호출
	else
	{										// 찾고자 하는 값과 일치할 때
		point = root;					// point에 root가 가리키는 것을 공유
		found = true;						// found값을 true로 해서 찾는 과정을 멈춤
	}
}
